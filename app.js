...
(function() {
  ...
  d3.paratext = (function(_super) {
    ...
    paratext.prototype.makeMap = function() {
      var stamenAttribution, textControl;
      ...
      textControl = L.Control.extend({
        ...
        onAdd: function(map) {
          var disable3D;
          this._textDomEl = L.DomUtil.create('div', 'paratext-info');
          L.DomUtil.enableTextSelection(this._textDomEl);
          map.getPanes().overlayPane.appendChild(this._textDomEl);
          this._textDomObj = $(L.DomUtil.get(this._textDomEl));
          this._textDomObj.css('width', $(map.getContainer())[0].clientWidth / 3);
          this._textDomObj.css('height', $(map.getContainer())[0].clientHeight - 20);
  ...
  draw = function(data) {
    var $texts, count, d3text, map, para, paratextinfo, texts;
    count = 0;
    para = new d3.paratext().text(data);
    map = $("body").append("<div id='map'></div>");
    para.makeMap();
    $("body").append(para._m);
    paratextinfo = d3.select(".paratext-info");
    d3text = d3.select(".paratext-info").append("svg").attr("width", $(para._m.getContainer())[0].clientWidth / 3).attr("height", $(para._m.getContainer())[0].clientHeight - 20).append("g").selectAll("text");
    d3text.data(para.properties.text).enter().append("text").attr("width", $(para._m.getContainer())[0].clientWidth / 3).attr("height", $(para._m.getContainer())[0].clientHeight - 20).style("font-family", "Helvetica").attr("fill", "black").text(function(d) {
      return d.description;
    }).on("mouseover", function() {
      d3.select(this).transition().duration(300).style("fill", "gray");
    }).on("mouseout", function() {
